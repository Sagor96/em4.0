          ...
          @if(Cart::count()>0)
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody>
                      @foreach(Cart::content() as $f )
                        <tr>
                            <td>{{$f->model->fl_name}}</td>
                            <td><input class="form-control" type="text" value="{{$f->model->fl_qty}}" /></td>
                            <td class="text-right">{{$f->model->fl_cost}}</td>
                            <td class="text-right">
                              <form action="/cart/{{$f->rowId}}"method="post">
                                @csrf
                                @method('DELETE')
